...
interface CommandCenterProps {
  ...
  history: Array<{ label: string; query: string }>;
...
const CommandCenter = ({
  ...
  const themes = [
    { id: 'default', label: 'Default', colors: ['#3b82f6', '#10b981', '#8b5cf6'] },
    { id: 'corporate', label: 'Corporate', colors: ['#1e40af', '#1e3a8a', '#3730a3'] },
    { id: 'energy', label: 'Energy', colors: ['#16a34a', '#15803d', '#84cc16'] },
    { id: 'sunset', label: 'Sunset', colors: ['#dc2626', '#ea580c', '#f59e0b'] },
    { id: 'glassmorphic', label: 'Neon', colors: ['#60a5fa', '#34d399', '#a78bfa'] },
  ];
    ...
    >
      <div className="h-full glass-strong flex flex-col border-r border-border/30">
        ...
        <Button
          ...
          className="absolute -right-3 top-4 z-50 h-6 w-6 rounded-full glass border border-border/50"
        >
        ...
        <div className="flex flex-col items-center gap-1 py-2 border-b border-border/30">
          ...
          <Button
            ...
            size="icon"
            onClick={() => { setActiveTab('theme'); setIsCollapsed(false); }}
            className="h-9 w-9"
            title="Theme"
          >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-1">
                    {alignOptions.map((opt) => (
                      <Button
                        key={opt.id}
                        variant={settings.nodeAlign === opt.id ? 'secondary' : 'ghost'}
                        size="sm"
                        className="flex-1 text-xs h-7"
                        onClick={() => onSettingsChange({ ...settings, nodeAlign: opt.id as any })}
                      >
                        {opt.label}
                      </Button>
                    ))}
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    onValueChange={([val]) => onSettingsChange({ ...settings, linkOpacity: val })}
                    min={0.1}
                    max={1}
                    step={0.1}
                    className="w-full"
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => onSettingsChange({ ...settings, showConfidence: checked })}
                  />
                </div>
              </div>
            )}
                ...
                <div className="space-y-2">
                  {themes.map((t) => (
                    <button
                      ...
                      className={cn(
                        'w-full p-2 rounded-lg flex items-center gap-2 transition-all',
                        settings.theme === t.id
                  ...
                  <div className="space-y-1">
                    {history.map((entry, index) => (
                      <button
                        ...
                        className="w-full text-left p-2 rounded-lg text-xs hover:bg-muted/50 transition-colors truncate"
                      >
                        {entry.label}
                      </button>
                    ))}
